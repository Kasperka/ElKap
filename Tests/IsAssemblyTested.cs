...
namespace ElKap.Tests
{
    public abstract class IsAssemblyTested : TestAsserts
	{
        ...
        private static bool isCorrectTest(Type y) => isCorrectlyInherited(y) && isTestClass(y);

        private static bool isTestClass(Type x) => x.IsInherited(typeof(IsTypeTested));

        private static bool isCorrectlyInherited(Type x) => x?.HasAttribute<TestClassAttribute>() ?? false;
